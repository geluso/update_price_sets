...
from zip_codes import FORCE_REDO_ZONES, WA_ZIP_CODE_NAMES, zone_to_zips
...
def loadsheet():
    ...
    distance_sheet = []
    with open('./csv/2025-03-26-18-45-distances.csv', newline='') as csvfile:
...
def main():
    ...
    while count < max_samples:
        count += 1
        print(count, "of", max_samples)
        zone_pick, zips_pick = random.choice(WA_ZIP_CODE_NAMES)
        zone_drop, zips_drop = random.choice(WA_ZIP_CODE_NAMES)

        sheet_distance_miles = get_sheet_distance(distance_sheet, zone_pick, zone_drop)
        fetched_distance_meters = fetch_zip_zip_distance(zone_pick, zone_drop, zips_pick, zips_drop)
        fetched_distance_miles = fetched_distance_meters / 1600
        difference = sheet_distance_miles - fetched_distance_miles

        id = "%04d %d %d %s to %s" % (difference, sheet_distance_miles, fetched_distance_miles, zone_pick, zone_drop)
        sorted.append(id)
        print(id)
    ...
    now = datetime.now()
    filename = f'./out/{now.year}-{now.month:02d}-{now.day:02d}-{now.hour:02d}-{now.minute:02d}-random_sampler.text'
    with open(filename, "w") as crow_file:
        crow_file.write("Difference | Saved Distance | Reobtained Distance | Pick Zone | Drop Zone\n")
        for item in sorted:
            print(item)
            crow_file.write(item + "\n")
