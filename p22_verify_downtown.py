import random

import csv
import math
from collections import defaultdict
from datetime import datetime
import time

from db import create_default_connection, count_rows_zip_zip_distance, get_zip_zip_distance, \
    insert_zip_zip_distance_pick_drop_distance_miles
from p10_fill_distance_sheet_holes import fetch_zip_zip_distance, save_zip_zip_distances, \
    zips_to_real_longitude_latitude
from wa_zip_gps import WA_ZIP_CODE_GPS
from zip_codes import FORCE_REDO_ZONES, WA_ZIP_CODE_NAMES, zone_to_zips


def gps_distance(gps_pick, gps_drop):
    latitude_pick, longitude_pick = gps_pick
    latitude_drop, longitude_drop = gps_drop
    yy = (latitude_pick - latitude_drop) * 110.574
    xx = (longitude_pick - longitude_drop) * 111.320 * 0.68199836006
    distance_km = math.sqrt(yy * yy + xx * xx)
    miles = distance_km / 1.6
    return miles

def loadsheet():
    print("Reading distance sheet...")
    distance_sheet = []
    with open('./csv/2025-03-26-18-45-distances.csv', newline='') as csvfile:
        price_sets = csv.reader(csvfile)
        for row in price_sets:
            distance_sheet.append(row)
    return distance_sheet

def get_sheet_distance(sheet, zone_pick, zone_drop):
    pick_col = 0
    drop_row = 0

    pick_i = 0
    while pick_col < len(sheet[0]):
        if sheet[0][pick_i] == zone_pick:
            pick_col = pick_i
            break
        pick_i += 1

    drop_i = 0
    while drop_i < len(sheet):
        if sheet[0][drop_i] == zone_drop:
            drop_row = drop_i
            break
        drop_i += 1

    return float(sheet[drop_row][pick_i])

def main():
    distance_sheet = loadsheet()
    sorted = []

    count = 0
    for zone_drop, zips_drop in WA_ZIP_CODE_NAMES:
        count += 1
        print(count, "of", len(WA_ZIP_CODE_NAMES))
        zone_pick, zips_pick =  ['SEATTLE:DOWNTOWN', [98101,98104,98121,98154,98174,98164]]

        sheet_distance_miles = get_sheet_distance(distance_sheet, zone_pick, zone_drop)
        fetched_distance_meters = fetch_zip_zip_distance(zone_pick, zone_drop, zips_pick, zips_drop)
        fetched_distance_miles = fetched_distance_meters / 1600
        difference = sheet_distance_miles - fetched_distance_miles

        id = "%04d %d %d %s to %s" % (difference, sheet_distance_miles, fetched_distance_miles, zone_pick, zone_drop)
        sorted.append(id)
        print(id)

    sorted.sort(reverse=True)

    now = datetime.now()
    filename = f'./out/{now.year}-{now.month:02d}-{now.day:02d}-{now.hour:02d}-{now.minute:02d}-verify_downtown.text'
    with open(filename, "w") as crow_file:
        crow_file.write("Difference | Saved Distance | Reobtained Distance | Pick Zone | Drop Zone\n")
        for item in sorted:
            print(item)
            crow_file.write(item + "\n")

if __name__ == "__main__":
    main()
    print('exit')