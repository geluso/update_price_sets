...
def main():
    ...
    tally_miles_off = defaultdict(lambda: 0)
    ...
    for zone_pick, zips_pick in WA_ZIP_CODE_NAMES:
        for zone_drop, zips_drop in WA_ZIP_CODE_NAMES:
            gps_pick = zips_to_real_longitude_latitude(zips_pick)
            gps_drop = zips_to_real_longitude_latitude(zips_drop)
            miles = gps_distance(gps_pick, gps_drop)
            sheet_distance = get_sheet_distance(distance_sheet, zone_pick, zone_drop)
            ratio = 1
            if sheet_distance != 0:
                ratio = miles / sheet_distance
            miles_off = round(sheet_distance) - round(miles)
            tally_miles_off[miles_off] += 1

            id = "%05d %04d %04d %s to %s" % (miles_off, miles, sheet_distance, zone_pick, zone_drop)
            sorted.append(id)

            print(miles_off, ratio)

    for miles_off in tally_miles_off:
        print(miles_off, tally_miles_off[miles_off])

    sorted.sort(reverse=True)

    now = datetime.now()
    crow_filename = f'./out/{now.year}-{now.month:02d}-{now.day:02d}-{now.hour:02d}-{now.minute:02d}-crow-distances.text'
    with open(crow_filename, "w") as crow_file:
        crow_file.write("Miles Difference | Crow Miles | Road Miles | Pick Zone | Drop Zone\n")
        for item in sorted:
            print(item)
