...
def main():
    ...
    for zone_pick, zips_pick in WA_ZIP_CODE_NAMES:
        for zone_drop, zips_drop in WA_ZIP_CODE_NAMES:
            ...
            id = "%05d %s to %s" % (sheet_distance, zone_pick, zone_drop)
            sorted.append(id)

    sorted.sort(reverse=True)

    now = datetime.now()
    filename = f'./out/{now.year}-{now.month:02d}-{now.day:02d}-{now.hour:02d}-{now.minute:02d}-max-distances.text'
    with open(filename, "w") as crow_file:
        crow_file.write("Total Road Miles | Pick Zone | Drop Zone\n")
        for item in sorted:
            print(item)
