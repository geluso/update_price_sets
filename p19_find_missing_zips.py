...
def main():
    ...
    missing_zips = set()
    with open("./notes/large_distances.txt") as large_distances:
        ...
        for line in lines[2:-1]:
            ...
            if zip_pick not in zip_to_longitude_latitude:
                ...
                missing_zips.add(zip_pick)
            if zip_drop not in zip_to_longitude_latitude:
                print("NO GPS", zip_drop)
                missing_zips.add(zip_drop)

    sorted_missing_zips = list(missing_zips)
    sorted_missing_zips.sort()

    for missing_zip in sorted_missing_zips:
        print(missing_zip)
