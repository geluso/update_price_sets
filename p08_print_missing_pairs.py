...
def main():
    ...
    for pick_zip, lat, longg in WA_ZIP_CODE_GPS:
        print("progress", count / total, missing)
        for drop_zip, lat, longg in WA_ZIP_CODE_GPS:
            ...
            if rows == 0 and rows_reversed == 0:
                ...
                missing_zone_to_zones.add((min_zone, max_zone, pick_zip, drop_zip))
    for index, missing_route in enumerate(missing_zone_to_zones):
        print(index, missing_route)
    print(len(missing))
